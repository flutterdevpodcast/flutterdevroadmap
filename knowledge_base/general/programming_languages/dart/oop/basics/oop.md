...
Пример явного сеттера. Явный сеттер можно написать только для приватного поля.
Это сделано, чтобы закрытое поле можно было изменять по правилам, описанным в сеттере.

![setter](oop_sample_5.png)

### Конструкторы

Каждый класс создает объект. Объект, перед тем как начать свою работу - инициализируется в памяти.
При инициализации объекта, первая функция которая срабатывает внутри него - конструктор(инициализатор).
Он позволяет задать ряд параметров, которые необходимы объекту для работы.
...
Для этого можно воспользоваться именнованными конструкторами.
...
Если у родительского типа будет только именнованный конструктор - вызывать придется его

![super named](opp_sample_10.png)

### Список инициализации
Иногда бывает необходимо инициализировать какие-то переменные до того, как конструктор начнет выполнение.
...
, пока приложение не завершит выполнение. Не смотря на то, что эти объекты описаны
